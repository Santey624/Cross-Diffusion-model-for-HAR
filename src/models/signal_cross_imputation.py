# ============================================================
# Cross-Sensor Signal Imputation Network (no VAE, no Diffusion)
#
# Direct regression: p(x_missing | x_observed) via MSE loss.
# All sensors interpolated to T_COMMON=256 for cross-attention.
#
# Architecture mirrors signal_cross_diffusion but removes the
# diffusion components (no timestep, no noise schedule).
# ============================================================
...
# Main model
# ============================================================
class SignalCrossImputation(nn.Module):
    """
    Cross-sensor imputation network — direct MSE regression.

    Given observed sensor signals, directly predicts missing sensor signals.
    No diffusion, no noise schedule, single forward pass at inference.
    ...
    def forward(self, signals, observed_mask):
        """
        signals:       (B, K, C, T_COMMON) — observed=real signal, missing=zeros
        observed_mask: (B, K) — 1=observed, 0=missing

        Returns: pred (B, K, C, T_COMMON) — prediction for ALL sensors
        """
        B, K, C, T = signals.shape
        device = signals.device

        # (B, K, C, T) → (B, K, T, C)
        x = signals.permute(0, 1, 3, 2)
...
def create_signal_cross_imputation(n_sensors=7, in_channels=3, d_model=128,
                                    num_heads=4, num_blocks=6, dropout=0.1):
    return SignalCrossImputation(
