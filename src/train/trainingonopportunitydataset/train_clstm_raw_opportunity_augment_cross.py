# ============================================================
# Train C-LSTM-A on Raw Signals + Cross-Sensor Diffusion Augmentation
#                                                         — Opportunity
#
# Same model & recipe as train_clstm_raw_opportunity.py, but
# --augment-cross is ALWAYS enabled:
#   30% of training batches: randomly hide 1-3 sensors and fill them
#   with the frozen Opportunity cross-sensor diffusion model.
#
# This trains a robustness classifier that has seen imputed sensors,
# for fairer downstream evaluation of CrossDiff.
#
# Does NOT overwrite the original real-only classifiers:
#   old:  checkpoints/clstm_raw_opportunity_{track}/
#   new:  checkpoints/clstm_raw_opportunity_{track}_augment_cross/
#
# Flags:
#   --track NAME   label track to classify (default: locomotion)
#   --tcommon      resample ALL sensors to T_COMMON=256
#
# Prereq:
#   checkpoints/opportunity_signal_cross_diffusion/best_model.pt
#   checkpoints/opportunity_signal_cross_diffusion/normalization_stats.pt
#   preprocessed Opportunity arrays
#
# Usage (from repo root):
#   python -m src.train.trainingonopportunitydataset.train_clstm_raw_opportunity_augment_cross
#   python -m src.train.trainingonopportunitydataset.train_clstm_raw_opportunity_augment_cross --track locomotion
#   python -m src.train.trainingonopportunitydataset.train_clstm_raw_opportunity_augment_cross --track hl_activity
# ============================================================
...
# Always-on cross-sensor diffusion augmentation
AUGMENT_CROSS  = True
AUG_PROB_CROSS = 0.3
DDIM_STEPS     = 10
...
def main():
    print(f"\n{'='*65}")
    print(f"Training C-LSTM-A + CrossDiff Augment — Opportunity ({tag})")
    print(f"AugmentCross: ALWAYS ON (p={AUG_PROB_CROSS}) | Device: {DEVICE}")
    print(f"Output: {OUT_DIR}")
    print(f"{'='*65}\n")

    if not (CROSS_DIFF_DIR / "best_model.pt").exists():
        raise FileNotFoundError(
            f"Missing diffusion checkpoint: {CROSS_DIFF_DIR / 'best_model.pt'}\n"
            "Train Opportunity cross-sensor diffusion first."
        )
    ...
    # Frozen cross-sensor diffusion for augmentation
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        for batch in tqdm(train_loader, desc=f"[Train] {epoch}/{EPOCHS}", leave=False):
            ...
            if random.random() < AUG_PROB_CROSS:
    ...
    print("Note: original real-only classifiers were not modified.")
