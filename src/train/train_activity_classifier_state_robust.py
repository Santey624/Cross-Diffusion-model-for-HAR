# ============================================================
# Train Robust Activity Classifier for STATE Activities (6 classes)
# Mixed training: real latents + diffusion-imputed latents
# ============================================================
...
OUT_DIR = Path("checkpoints/activity_classifier_state_robust")
...
def ddim_sample_v2_fast(model, stacked_latents, observed_mask, alpha_bar, T, ddim_steps=20):
    ...
    for i in range(len(tau) - 1):
        ...
        pred_x0 = torch.clamp(
            (z - torch.sqrt(1 - ab_now) * noise_pred) / torch.sqrt(ab_now), -5.0, 5.0
        )
...
def main():
    print(f"\n{'='*60}")
    print("Training Robust State Activity Classifier (6 classes)")
    ...
    sched = make_schedule(T, diff_ckpt["schedule"])
