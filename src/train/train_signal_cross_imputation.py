# ============================================================
# Train Cross-Sensor Signal Imputation Network (no VAE, no Diffusion)
#
# Direct MSE regression: p(x_missing | x_observed)
# All sensors interpolated to T_COMMON=256 for cross-attention.
#
# Usage:
#   python -m src.train.train_signal_cross_imputation
# ============================================================
...
from src.models.signal_cross_imputation import (
    create_signal_cross_imputation, T_COMMON,
)
...
OUT_DIR = Path("checkpoints/signal_cross_imputation")
...
def main():
    print(f"\n{'='*65}")
    print(f"Training Signal Cross-Sensor Imputation (MSE regression)")
    print(f"T_COMMON={T_COMMON}, Epochs={EPOCHS}")
    ...
    # Model
    print(f"\nCreating Signal Cross-Sensor Imputation model...")
    model = create_signal_cross_imputation(
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        for batch in tqdm(train_loader, desc=f"[Train] {epoch}/{EPOCHS}", leave=False):
            ...
            # Zero out missing sensors in input
            inp = stacked_norm.clone()
            for i in missing_idx:
                inp[:, i] = 0.0

            pred = model(inp, observed_mask)   # (B, K, C, T_COMMON)

            # Loss only on missing sensors
            loss = sum(
                F.mse_loss(pred[:, i], stacked_norm[:, i])
                for i in missing_idx
            ) / len(missing_idx)
