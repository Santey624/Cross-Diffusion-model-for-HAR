# ============================================================
# Train C-LSTM-A Classifier on Decoded Signals — Behavioral (54/55 classes)
#
# Option 2 pipeline:
...
OUT_DIR = Path("checkpoints/clstm_behavioral_robust" if ROBUST else "checkpoints/clstm_behavioral")
OUT_DIR.mkdir(parents=True, exist_ok=True)

BATCH_SIZE = 32
EPOCHS = 100
LR = 1e-3
MASK_PROB = 0.5   # fraction of batches with random masking (robust mode)


def decode_latents(vae, latents):
    """Decode latents dict to signals dict in (B, C, T) format."""
    decoded = {}
...
def main():
    print(f"\n{'='*60}")
    print(f"Training C-LSTM-A — Behavioral Activities")
    ...
    # Pre-compute mean latents from training set (for robust masking)
    mean_latents = None
    if ROBUST:
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        for batch in tqdm(train_loader, desc=f"[Train] Epoch {epoch}/{EPOCHS}", leave=False):
            ...
            with torch.no_grad():
                ...
                # Robust: randomly mask sensors with mean-fill
