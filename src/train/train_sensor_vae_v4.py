...
def main():
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        t_tot = t_rec = t_kls = t_klp = t_imp = 0.0
        n_cogage = n_wisdm = 0
        ...
        for batch in tqdm(train_loader, desc=f"[Train] {epoch}/{EPOCHS}", leave=False):
            ...
            with torch.cuda.amp.autocast():
                ...
                # --- Imputation loss: encode all-but-one, decode missing ---
        ...
        N = len(train_loader)
        t_tot /= N; t_rec /= N; t_kls /= N; t_klp /= N; t_imp /= N
        ...
        with torch.no_grad():
            for batch in tqdm(test_loader, desc=f"[Eval ] {epoch}/{EPOCHS}", leave=False):
                ...
                with torch.cuda.amp.autocast():
                    ...
                    # Eval imputation: try each sensor once
                    imp_loss_sum = 0.0
                    for target in SENSOR_NAMES:
                        others = {k: sensor_data[k] for k in SENSOR_NAMES if k != target}
                        imputed = model.impute(others, target)
                        imp_loss_sum += F.mse_loss(imputed, sensor_data[target]).item()
        ...
        print(
            ...
            f"  Train: loss={t_tot:.4f} recon={t_rec:.4f} kl_s={t_kls:.4f} kl_p={t_klp:.4f} impute={t_imp:.4f}\n"
            f"  Test : loss={e_tot:.4f} recon={e_rec:.4f} kl_s={e_kls:.4f} kl_p={e_klp:.4f} impute={e_imp:.4f}\n"
        )
        ...
        if epoch >= KL_WARMUP_EPOCHS and e_rec < best_recon:
            best_recon = e_rec
