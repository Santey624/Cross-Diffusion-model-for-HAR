# ============================================================
# Evaluate Classifier-Guided Diffusion for Sensor Imputation
# Unconditional DM + Classifier Gradient Guidance
# ============================================================

from pathlib import Path
import math
import torch
import torch.nn.functional as F
import matplotlib.pyplot as plt
from tqdm import tqdm

from src.models.sensor_vae import SensorMultiModalVAE, SENSOR_NAMES
from src.models.sensor_uncond_diffusion import create_uncond_diffusion_model
from src.models.sensor_guidance_classifier import create_guidance_classifier


# ============================================================
# CONFIG
# ============================================================
DEVICE = "cuda" if torch.cuda.is_available() else "cpu"

VAE_CHECKPOINT = "checkpoints/sensor_vae_epoch_047.pt"
UNCOND_DM_DIR = Path("checkpoints/uncond_diffusion")
CLASSIFIER_DIR = Path("checkpoints/guidance_classifier")
LATENTS_DIR = Path("data/sensor_latents")

NUM_EVAL_SAMPLES = 100
DDIM_STEPS = 200
GUIDANCE_SCALE = 1.0  # Gradient weight

OUTPUT_DIR = Path("outputs/guided_imputation_eval")
OUTPUT_DIR.mkdir(parents=True, exist_ok=True)

SENSOR_NAME_TO_IDX = {name: i for i, name in enumerate(SENSOR_NAMES)}

# Evaluation scenarios (same as conditional eval)
SCENARIOS = {
    "single_phone_acc":   {"missing": ["phone_acc"]},
    "single_phone_gyro":  {"missing": ["phone_gyro"]},
    "single_phone_grav":  {"missing": ["phone_grav"]},
    "single_phone_lacc":  {"missing": ["phone_lacc"]},
    "single_watch_acc":   {"missing": ["watch_acc"]},
    "single_watch_gyro":  {"missing": ["watch_gyro"]},
    "single_glasses_acc": {"missing": ["glasses_acc"]},
    "device_phone":   {"missing": ["phone_acc", "phone_gyro", "phone_grav", "phone_lacc"]},
    "device_watch":   {"missing": ["watch_acc", "watch_gyro"]},
    "device_glasses": {"missing": ["glasses_acc"]},
}


# ============================================================
# COSINE SCHEDULE
# ============================================================
def cosine_beta_schedule(T, s=0.008):
    steps = torch.arange(T + 1, dtype=torch.float64)
    f_t = torch.cos((steps / T + s) / (1 + s) * math.pi / 2) ** 2
    alpha_bar = f_t / f_t[0]
    betas = 1 - (alpha_bar[1:] / alpha_bar[:-1])
    betas = torch.clamp(betas, min=1e-6, max=0.999)
    return betas.float()


def make_schedule(T, schedule_type):
    if schedule_type == "cosine":
        betas = cosine_beta_schedule(T)
    else:
        betas = torch.linspace(1e-4, 0.02, T)
    alphas = 1.0 - betas
    alpha_bar = torch.cumprod(alphas, dim=0)
    return {"alpha_bar": alpha_bar}


# ============================================================
# GUIDED DDIM SAMPLER
# ============================================================
def guided_ddim_sample(
    uncond_model,
    classifier,
    target_modality,
    shape,
    observed_conditions,
    alpha_bar,
    T,
    ddim_steps=200,
    guidance_scale=1.0,
):
    """
    DDIM sampling with classifier guidance.

    At each step:
    1. Get unconditional noise prediction
    2. Compute gradient of classifier loss w.r.t. z_t
    3. Shift noise prediction: noise_pred = noise_pred + scale * grad
    """
    B = shape[0]
    device = next(uncond_model.parameters()).device
    z = torch.randn(shape, device=device)
    alpha_bar = alpha_bar.to(device)

    sensor_idx = SENSOR_NAME_TO_IDX[target_modality]

    # Quadratic spacing
    tau = torch.linspace(0, 1, ddim_steps + 1, device=device)
    tau = (tau ** 2 * (T - 1)).long()
    tau = tau.flip(0)

    for i in tqdm(range(len(tau) - 1), desc=f"Guided DDIM {target_modality}", leave=False):
        t_now = tau[i]
        t_next = tau[i + 1]
        t_batch = torch.full((B,), t_now, device=device, dtype=torch.long)

        # Unconditional noise prediction
        with torch.no_grad():
            noise_pred = uncond_model(z_t=z, t=t_batch, sensor_idx=sensor_idx)

        # Classifier guidance gradient
        if guidance_scale > 0 and len(observed_conditions) > 0:
            z_grad = z.detach().requires_grad_(True)

            # Classifier predicts all sensors from noisy z
            preds = classifier(z_t=z_grad, t=t_batch, sensor_idx=sensor_idx)
            # preds: (B, 7, D, T)

            # Loss: MSE between predicted and observed conditions
            loss = 0.0
            count = 0
            for name, obs in observed_conditions.items():
                if obs is not None:
                    idx = SENSOR_NAME_TO_IDX[name]
                    pred_i = preds[:, idx]  # (B, D, T)
                    loss = loss + F.mse_loss(pred_i, obs)
                    count += 1

            if count > 0:
                loss = loss / count
                grad = torch.autograd.grad(loss, z_grad)[0]

                # Guidance: shift noise prediction to reduce classifier loss
                # The gradient points toward higher loss, so we subtract
                noise_pred = noise_pred + guidance_scale * grad

        # DDIM step
        ab_now = alpha_bar[t_now]
        ab_next = alpha_bar[t_next]

        pred_x0 = (z - torch.sqrt(1 - ab_now) * noise_pred) / torch.sqrt(ab_now)
        pred_x0 = torch.clamp(pred_x0, -5.0, 5.0)

        dir_zt = torch.sqrt(1 - ab_next) * noise_pred
        z = torch.sqrt(ab_next) * pred_x0 + dir_zt

    return z


# ============================================================
# MAIN
# ============================================================
def main():
    print(f"\n{'='*60}")
    print("Evaluating Classifier-Guided Diffusion")
    print(f"DDIM Steps: {DDIM_STEPS}, Guidance Scale: {GUIDANCE_SCALE}")
    print(f"{'='*60}\n")

    # Load unconditional DM
    ckpt_dm = torch.load(UNCOND_DM_DIR / "best_model.pt", map_location=DEVICE)
    T = ckpt_dm["T"]
    schedule_type = ckpt_dm["schedule"]
    cfg_dm = ckpt_dm["config"]
    print(f"Uncond DM: Epoch {ckpt_dm['epoch']}, Loss {ckpt_dm['loss']:.6f}")

    uncond_model = create_uncond_diffusion_model(
        hidden_dim=cfg_dm["hidden_dim"],
        num_heads=cfg_dm["num_heads"],
        num_conv_blocks=cfg_dm["num_conv_blocks"],
        num_attn_blocks=cfg_dm["num_attn_blocks"],
        dropout=0.0,
    ).to(DEVICE)
    uncond_model.load_state_dict(ckpt_dm["model_state"])
    uncond_model.eval()

    # Load classifier
    ckpt_cls = torch.load(CLASSIFIER_DIR / "best_model.pt", map_location=DEVICE)
    cfg_cls = ckpt_cls["config"]
    print(f"Classifier: Epoch {ckpt_cls['epoch']}, Loss {ckpt_cls['loss']:.6f}")

    classifier = create_guidance_classifier(
        hidden_dim=cfg_cls["hidden_dim"],
        num_conv_blocks=cfg_cls["num_conv_blocks"],
        dropout=0.0,
    ).to(DEVICE)
    classifier.load_state_dict(ckpt_cls["model_state"])
    classifier.eval()

    # Load VAE
    print("Loading Sensor VAE...")
    vae = SensorMultiModalVAE().to(DEVICE)
    vae_ckpt = torch.load(VAE_CHECKPOINT, map_location=DEVICE)
    vae.load_state_dict(vae_ckpt["model_state"])
    vae.eval()

    # Load latents
    print("Loading sensor latents...")
    latents = {}
    for name in SENSOR_NAMES:
        latents[name] = torch.load(LATENTS_DIR / f"train_latents_{name}_mu.pt")[:NUM_EVAL_SAMPLES]
        print(f"  {name:15s}: {latents[name].shape}")

    # Load normalization stats
    norm_stats = torch.load(UNCOND_DM_DIR / "normalization_stats.pt", map_location=DEVICE)

    latents_norm = {}
    for name in SENSOR_NAMES:
        mean = norm_stats[name]["mean"].to(DEVICE)
        std = norm_stats[name]["std"].to(DEVICE)
        latents_norm[name] = (latents[name].to(DEVICE) - mean) / std

    # Build schedule
    sched = make_schedule(T, schedule_type)

    # Run each scenario
    all_results = {}

    for scenario_name, scenario_cfg in SCENARIOS.items():
        missing = scenario_cfg["missing"]
        print(f"\n{'='*60}")
        print(f"Scenario: {scenario_name}")
        print(f"Missing sensors: {missing}")
        print(f"{'='*60}")

        scenario_results = {}

        for target_name in missing:
            # Observed conditions: all except missing sensors
            observed_conditions = {}
            for name in SENSOR_NAMES:
                if name not in missing:
                    observed_conditions[name] = latents_norm[name]

            # Sample with guidance
            target_shape = latents_norm[target_name].shape
            imputed_norm = guided_ddim_sample(
                uncond_model=uncond_model,
                classifier=classifier,
                target_modality=target_name,
                shape=target_shape,
                observed_conditions=observed_conditions,
                alpha_bar=sched["alpha_bar"],
                T=T,
                ddim_steps=DDIM_STEPS,
                guidance_scale=GUIDANCE_SCALE,
            )

            # Denormalize
            mean = norm_stats[target_name]["mean"].to(DEVICE)
            std = norm_stats[target_name]["std"].to(DEVICE)
            imputed = imputed_norm * std + mean

            # Latent MSE
            gt = latents[target_name].to(DEVICE)
            latent_mse = F.mse_loss(imputed, gt).item()

            # Decode with VAE
            with torch.no_grad():
                imputed_signals = vae.decode_sensor(target_name, imputed).cpu()
                gt_signals = vae.decode_sensor(target_name, gt).cpu()

            signal_mse = F.mse_loss(imputed_signals, gt_signals).item()

            scenario_results[target_name] = {
                "latent_mse": latent_mse,
                "signal_mse": signal_mse,
            }

            print(f"  {target_name:15s}: Latent MSE={latent_mse:.6f}, Signal MSE={signal_mse:.6f}")

            # Visualize first 3 samples
            n_ch = imputed_signals.shape[2]
            for idx in range(min(3, imputed_signals.shape[0])):
                fig, axs = plt.subplots(n_ch, 1, figsize=(14, 3 * n_ch))
                if n_ch == 1:
                    axs = [axs]
                for ch in range(n_ch):
                    axs[ch].plot(gt_signals[idx, :, ch].numpy(), label="Ground Truth", alpha=0.8)
                    axs[ch].plot(imputed_signals[idx, :, ch].numpy(), label="Guided Diff",
                                 alpha=0.8, linestyle='--')
                    axs[ch].set_title(f"{target_name} Ch{ch}")
                    axs[ch].legend()
                    axs[ch].grid(alpha=0.3)
                plt.tight_layout()
                plt.savefig(OUTPUT_DIR / f"{scenario_name}_{target_name}_sample_{idx}.png", dpi=150)
                plt.close()

        all_results[scenario_name] = scenario_results

    # Summary
    print(f"\n{'='*60}")
    print("SUMMARY - Classifier-Guided Diffusion")
    print(f"{'='*60}")

    for scenario_name, results in all_results.items():
        print(f"\n  {scenario_name}:")
        avg_latent = sum(r["latent_mse"] for r in results.values()) / len(results)
        avg_signal = sum(r["signal_mse"] for r in results.values()) / len(results)
        for sensor, r in results.items():
            print(f"    {sensor:15s}: L-MSE={r['latent_mse']:.6f}  S-MSE={r['signal_mse']:.6f}")
        if len(results) > 1:
            print(f"    {'AVG':15s}: L-MSE={avg_latent:.6f}  S-MSE={avg_signal:.6f}")

    # Save metrics
    with open(OUTPUT_DIR / "all_metrics.txt", "w") as f:
        f.write(f"Guidance Scale: {GUIDANCE_SCALE}\n")
        for scenario_name, results in all_results.items():
            f.write(f"\n{scenario_name}:\n")
            for sensor, r in results.items():
                f.write(f"  {sensor}: latent_mse={r['latent_mse']:.6f}, "
                        f"signal_mse={r['signal_mse']:.6f}\n")

    print(f"\nSaved to: {OUTPUT_DIR}")
    print(f"{'='*60}\n")


if __name__ == "__main__":
    main()
