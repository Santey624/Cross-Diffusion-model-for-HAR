# ============================================================
# Evaluate C-LSTM-A Classifier — State Activities (6 classes)
#
# Option 2 pipeline:
...
# ============================================================
# HELPERS (identical to behavioral eval)
# ============================================================
...
def main():
    ...
    if ROBUST:
        CLASSIFIER_CKPT = CLASSIFIER_CKPT.replace("clstm_state", "clstm_state_robust")

    diff_version = "v3" if USE_V3 else "v2"

    print(f"\n{'='*70}")
    print(f"C-LSTM-A STATE EVAL | Diffusion: {diff_version} | Robust: {ROBUST}")
    print(f"{'='*70}\n")
    ...
    if not Path(CLASSIFIER_CKPT).exists():
        ...
        print("Please train it first with: python -m src.train.train_clstm_state")
        return
    ...
    # ============================================================
    print(f"\n{'='*100}")
    print(f"C-LSTM-A — STATE ACTIVITIES (6 classes)  |  Diffusion: {diff_version}")
    print(f"{'='*100}")
