...
def main():
    ...
    MLP_CHECKPOINT = "checkpoints/activity_classifier_state_mlp/best_model.pt"
    mlp_classifier = None
    if Path(MLP_CHECKPOINT).exists():
        ...
        mlp_classifier = create_activity_classifier(
            model_type="mlp", n_classes=test_dataset.n_classes,
    ...
    print(f"\n{'='*110}")
    print(f"STATE ACTIVITIES — COMPARISON BY PATTERN (guidance_scale={GUIDANCE_SCALE})")
    print(f"{'='*110}")
